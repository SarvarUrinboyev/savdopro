...
/**
 * A read-only suggestion from the GLOBAL barcode lookup (Open Food Facts / UPC
 * Item DB), used by the warehouse scan modal as a fallback when the national
 * catalogue has nothing for an unknown code.
 *
 ...
 *
 * @param source which database supplied the hit: {@code "openfoodfacts"} or
 *               {@code "upcitemdb"} (null on a miss).
 */
